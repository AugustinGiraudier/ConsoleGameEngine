...
int main() {
	...
	{ 
		Hint_Window_Size hws(My_Size_X, My_Size_Y);		// fenetre de 130 par 30 caract�res
		Hint_Update_Period hup(Time_Between_Update);	// 20 millisecondes entre chaque affichage
		Hint_Default_Char hdc(' ');						// caract�re ' ' par d�faut
		Hint_Repeat_Function hrf(test);					// fonction � appeler �chaque boucle
		CW->Set_Hint(&hws);
		CW->Set_Hint(&hup);
		CW->Set_Hint(&hdc);
		//CW->Set_Hint(&hrf);
	...
	//on parcours tout l'�cran (sauf le contour) avec un '>' qui se d�place :
	for (int y = 1; y < CW->Get_Dim_Y()-1; ++y) {
		...
		CW->Set_Char_At(' ', CW->Get_Dim_X() - 2, y); // on efface l'ancien (� la ligne pr�c�dente)
	}
